	...
	const sendVerifyCode = async () => {
		try {
			if(data.email !== '') {
				const { phone } = data
				console.log(phone)
				const response = await axios.post(`${DEV_SERVER}/Account/sendSMSAuth`, { phone, type: 'PASSWORD' })
			...
			<SafeAreaView style={{ flex: 1 }}>
				...
				>
					...
					<View style={{ height: 50, flexDirection: "row", marginVertical: 10 }}>
						...
						>
							<TextInput placeholder="이메일을 입력해주세요" onChangeText={(text) => onChange(text, 'email')}/>
						</View>
					</View>
					...
					<View style={{ height: 50, flexDirection: "row", marginVertical: 10 }}>
						...
						>
							<TextInput placeholder="인증번호 입력" onChangeText={(text) => onChange(text, 'code')}/>
							<TouchableOpacity
									style={{
										display: "flex",
										justifyContent: "center",
